<?php
    ...
    */
    ...
    foreach ($repository->allWithMenuAndParent() as $page) {
        if (! $page->redirect) {
            if ($page->isPlugin()) {
                Route::get(makeSlug($page))->name($page->slug())->uses(userPluginController($page->slug(), 'index'));
            } else {
                Route::get(makeSlug($page))->name($page->slug())->uses('App\Http\Controllers\PageController@view');
            }
        }
    }
